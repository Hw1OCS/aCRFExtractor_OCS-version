#' This is the main program which does the page extraction from aCRF page, related to Value level. 
#'
...
crf_pageIn = crf_preprocess
defineOrigin_valueLevelTab = defineOrigin_valueLevelTab_preprocess
# domain_list = domain_DefOrigin_valueTab_filtered
...
get_valueLevel_pgNbr <- function (crf_pageIn = NULL,
  ...
  message("Page extraction for Value Level Tab is just stared: ")
  ...
  for (curr_row in 1:total_rows) {
    ...
    ## create output page number holder
    pages_all <- character()
    
    ## loop through the whole aCRF and grab page numbers corresponding to the last string
  ...
  ## combine all outputs
  # ds_out_combined <- do.call("rbind", ds_out)
